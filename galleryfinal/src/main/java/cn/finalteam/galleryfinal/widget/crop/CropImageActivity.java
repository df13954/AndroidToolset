...
//                setCropSaveException(e);
    ...
    private class Cropper {
        ...
        private void makeDefault() {
            ...
            //如果，选择的图片，没有达到这个高度，或者宽度？裁剪的时候，会直接裁剪周边黑色
            {
                float ratio = (float) maxX / (float) maxY;
                Log.e(TAG, "自定义的比例：" + ratio );
                ...
                if (maxX <= width && maxY <= height) {
                    ...
                    RectF cropRect = new RectF(x, y, x + maxX, y + maxY);
                    hv.setup(imageView.getUnrotatedMatrix(), imageRect, cropRect, customHeight);
                    imageView.add(hv);

                    //x 在图片内    y，超越图片
                } else if (maxX <= width && maxY > height) {
                    //把图片的height 赋值，裁剪的高度
                    maxY = height;
                    //根据比例，调整裁剪的width
                    maxX = (int) (ratio * maxY);
                    ...
                    hv.setup(imageView.getUnrotatedMatrix(), imageRect, cropRect, customHeight);
                    imageView.add(hv);
                } else if (width < maxX && height < maxY) {
                    Toast.makeText(CropImageActivity.this,"清晰度未达到作为背景的标准，请重新选择",Toast.LENGTH_SHORT).show();
                    finish();
                }
            }
        }
